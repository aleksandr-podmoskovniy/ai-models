...
5. Check documentation source-of-truth:
   - generated docs entrypoints such as `docs/CR*.md` were not hand-written;
   - public schema changes were made in OpenAPI/API/CRD source surfaces before
     docs output changed;
   - hand-written docs do not duplicate generated schema tables;
   - docs tasks ran `make lint-docs` and, when generated docs are involved,
     `make render-docs` or the repo's matching render check.
6. Check plan hygiene:
...
7. If repo-local workflow surfaces changed (`AGENTS.md`, `.codex/*`, `.agents/skills/*`, `.codex/agents/*`, `docs/development/CODEX_WORKFLOW.ru.md`, `docs/development/TASK_TEMPLATE.ru.md`, `docs/development/REVIEW_CHECKLIST.ru.md`, `plans/README.md`), check them as one instruction system:
...
8. If controller code changed, check whether controller quality gates and corrective architecture rules were respected.
...
9. If the task was substantial or used delegation, confirm whether a final `reviewer` pass is still required.
10. Return only concrete findings, missing checks, and residual risks.
