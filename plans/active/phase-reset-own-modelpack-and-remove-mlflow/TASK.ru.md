...
- зафиксировать первый native publisher cut как bounded replacement:
  ...
  out, без stream-from-source ambitions в этом же slice;
...
- не делать в этом же bundle stream-from-source publisher; этот reset slice
  фиксирует только binary-free native OCI publication;
