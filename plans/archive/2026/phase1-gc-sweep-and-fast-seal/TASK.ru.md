...
Continuation на 2026-04-23 после live scheduled-GC inspection:

8. Scheduled stale sweep не делает startup/backfill catch-up:
   - `dmcr-cleaner gc run --schedule=*/20 * * * *` планирует только следующий
     future tick;
   - если `dmcr` rollout произошёл после очередного 20-минутного tick,
     исторический orphan residue ждёт следующий tick, хотя `gc check` уже
     умеет его видеть;
   - unconditional cleanup on every restart unsafe, потому что может создавать
     лишние maintenance cycles;
   - нужен bounded startup check, который enqueue'ит обычный scheduled request
     только если stale report реально содержит cleanup candidates и нет уже
     active/queued GC request.
...
- Scheduled stale sweep получает bounded startup backfill:
  - startup path не запускает destructive cleanup напрямую;
  - он делает report-only stale check на runner start и bounded retry при
    transient check failure;
  - scheduled request создаётся только при stale candidates и отсутствии
    других GC request secrets;
  - request сохраняет обычный activation-delay / maintenance-mode choreography.
- Default `dmcr.gc.schedule` теперь `*/20 * * * *`, чтобы periodic stale sweep
  не ждал daily window при обычной эксплуатации.
...
- Тесты покрывают:
  ...
  - schedule wiring;
  - startup backfill does not enqueue empty scheduled sweeps;
