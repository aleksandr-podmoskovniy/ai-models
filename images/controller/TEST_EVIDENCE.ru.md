...
- Proved properties:
  ...
  - workload-facing runtime contract exposes every model by Kubernetes
    resource name under `/data/modelcache/models/<model-name>` and uses only
    `AI_MODELS_MODELS_DIR` plus `AI_MODELS_MODELS` for runtime discovery;
    managed workload env does not expose internal artifact URIs, managed
    SharedDirect delivery reads stable model-name projections from inline CSI
    mounts that bind the ready digest model directory instead of the cache
    entry root,
    ...
    multi-model -> single-model and model-set-shrink transitions, and fails fast
