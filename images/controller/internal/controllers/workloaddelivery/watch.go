...
func indexWorkloadReferences(ctx context.Context, indexer client.FieldIndexer, kinds []workloadKind) error {
...
func (r *baseReconciler) mapRayClustersForRayService() handler.MapFunc {
	return func(ctx context.Context, object client.Object) []reconcile.Request {
		return r.rayClusterRequestsForService(ctx, object)
	}
}

func (r *baseReconciler) mapRayClustersForModel() handler.MapFunc {
	return func(ctx context.Context, object client.Object) []reconcile.Request {
		requests := r.listWorkloadRequests(ctx, rayClusterWorkloadKind, client.InNamespace(object.GetNamespace()), client.MatchingFields{modelReferenceIndexField: object.GetName()})
		for _, service := range r.listRayServicesByReference(ctx, client.InNamespace(object.GetNamespace()), client.MatchingFields{modelReferenceIndexField: object.GetName()}) {
			requests = append(requests, r.rayClusterRequestsForService(ctx, service)...)
		}
		return requests
	}
}

func (r *baseReconciler) mapRayClustersForClusterModel() handler.MapFunc {
	return func(ctx context.Context, object client.Object) []reconcile.Request {
		requests := r.listWorkloadRequests(ctx, rayClusterWorkloadKind, client.MatchingFields{clusterModelReferenceIndexField: object.GetName()})
		for _, service := range r.listRayServicesByReference(ctx, client.MatchingFields{clusterModelReferenceIndexField: object.GetName()}) {
			requests = append(requests, r.rayClusterRequestsForService(ctx, service)...)
		}
		return requests
	}
...
func (r *baseReconciler) rayClusterRequestsForService(ctx context.Context, service client.Object) []reconcile.Request {
	if service == nil {
		return nil
	}
	list := newRayClusterList()
	if err := r.client.List(ctx, list, client.InNamespace(service.GetNamespace())); err != nil {
		r.logMapListFailure(rayClusterWorkloadKind.kind, err)
		return nil
	}
	...
	for _, item := range items {
		cluster, ok := item.(client.Object)
		if !ok {
			r.logMapListFailure(rayClusterWorkloadKind.kind, errUnexpectedWorkloadListItem)
			return nil
		}
		if !rayServiceOwnsCluster(service, cluster) {
			continue
		}
...
func (r *baseReconciler) listRayServicesByReference(ctx context.Context, options ...client.ListOption) []client.Object {
	list := newRayServiceList()
	if err := r.client.List(ctx, list, options...); err != nil {
		r.logMapListFailure(rayServiceReferenceKind.kind, err)
		return nil
	}
	items, err := apimeta.ExtractList(list)
	if err != nil {
	...
	services := make([]client.Object, 0, len(items))
	for _, item := range items {
		service, ok := item.(client.Object)
		if !ok {
			r.logMapListFailure(rayServiceReferenceKind.kind, errUnexpectedWorkloadListItem)
			return nil
		}
