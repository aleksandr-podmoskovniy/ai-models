...
func TestProjectedHFTokenSupportsKnownKeys(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "token",
			data: map[string][]byte{"token": []byte("hf-token")},
		},
		{
			name: "HF_TOKEN",
			data: map[string][]byte{"HF_TOKEN": []byte("hf-token")},
		},
		{
			name: "HUGGING_FACE_HUB_TOKEN",
			data: map[string][]byte{"HUGGING_FACE_HUB_TOKEN": []byte("hf-token")},
		},
	}
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			token, err := projectedHFToken(secret)
			if err != nil {
				t.Fatalf("projectedHFToken() error = %v", err)
			}
			if got, want := string(token), "hf-token"; got != want {
