...
func TestEnsureProjectedImagePullSecretFromSourceNamespaceCopiesDockerConfig(t *testing.T) {
	t.Parallel()
	...
	kubeClient := fake.NewClientBuilder().
		...
		WithObjects(
			owner,
			testkit.NewOCIRegistryWriteAuthSecret("d8-ai-models", "ai-models-module-registry"),
		).
		Build()

	projectedSecretName, err := EnsureProjectedImagePullSecretFromSourceNamespace(
		context.Background(),
		kubeClient,
		scheme,
		owner,
		...
		types.UID("1111-2222"),
		"d8-ai-models",
		"ai-models-module-registry",
	)
	if err != nil {
		t.Fatalf("EnsureProjectedImagePullSecretFromSourceNamespace() error = %v", err)
	}

	expectedName, err := resourcenames.RuntimeImagePullSecretName(types.UID("1111-2222"))
	if err != nil {
		t.Fatalf("RuntimeImagePullSecretName() error = %v", err)
	}
	if projectedSecretName != expectedName {
		t.Fatalf("projected secret name = %q, want %q", projectedSecretName, expectedName)
	}

	projected := &corev1.Secret{}
	if err := kubeClient.Get(context.Background(), client.ObjectKey{Name: projectedSecretName, Namespace: "team-a"}, projected); err != nil {
		t.Fatalf("Get(projected image pull secret) error = %v", err)
	}
	if got, want := projected.Type, corev1.SecretTypeDockerConfigJson; got != want {
		t.Fatalf("projected type = %q, want %q", got, want)
	}
	if got, want := string(projected.Data[corev1.DockerConfigJsonKey]), `{"auths":{"registry.internal.local":{"username":"ai-models","password":"secret"}}}`; got != want {
		t.Fatalf("unexpected dockerconfigjson %q", got)
	}
}

func TestDeleteProjectedImagePullSecretDeletesProjectedSecret(t *testing.T) {
	t.Parallel()

	scheme := testkit.NewScheme(t)
	secretName, err := resourcenames.RuntimeImagePullSecretName(types.UID("1111-2222"))
	if err != nil {
		t.Fatalf("RuntimeImagePullSecretName() error = %v", err)
	}
	kubeClient := fake.NewClientBuilder().
		WithScheme(scheme).
		WithObjects(
			&corev1.Secret{ObjectMeta: metav1.ObjectMeta{Name: secretName, Namespace: "team-a"}},
		).
		Build()

	if err := DeleteProjectedImagePullSecret(context.Background(), kubeClient, "team-a", types.UID("1111-2222")); err != nil {
		t.Fatalf("DeleteProjectedImagePullSecret() error = %v", err)
	}

	if err := kubeClient.Get(context.Background(), client.ObjectKey{Name: secretName, Namespace: "team-a"}, &corev1.Secret{}); !apierrors.IsNotFound(err) {
		t.Fatalf("expected projected image pull secret to be deleted, got err=%v", err)
	}
}
