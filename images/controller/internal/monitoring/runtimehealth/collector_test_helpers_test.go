...
func assertMetricAbsent(t *testing.T, families []*dto.MetricFamily, name string, labels map[string]string) {
	t.Helper()
	...
	for _, family := range families {
		...
		for _, metric := range family.GetMetric() {
			if hasExactLabels(metric, labels) {
				t.Fatalf("metric %s with labels %v unexpectedly found", name, labels)
			}
		}
	}
}
