...
func New(logger *slog.Logger, options Options) (*App, error) {
	...
	if err := options.PublicationOps.Validate(); err != nil {
		return nil, err
	}
	if options.HFPublication.Enabled() && !options.PublicationOps.Enabled() {
	...
	return &App{
		...
		publicationOps: options.PublicationOps,
		logger:         logger,
		runtime:        runtimeOptions,
	}, nil
}
...
func (a *App) Run(ctx context.Context) error {
	...
	if err := publicationops.SetupWithManager(mgr, a.publicationOps); err != nil {
		return err
	}
	...
	if a.publicationOps.Enabled() {
		a.logger.Info(
			"controller publication operations configured",
			slog.String("publishPodNamespace", a.publicationOps.PublishPod.Namespace),
			slog.String("publishPodImage", a.publicationOps.PublishPod.Image),
		)
	}
