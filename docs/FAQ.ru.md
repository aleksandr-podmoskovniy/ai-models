...
Целевая модель явная: node-cache runtime ставится только на ноды, выбранные
для local cache, а placement workload'а остаётся вне ai-models. SharedDirect
workload должен быть запланирован самим workload'ом, ai-inference или внешним
scheduler'ом на ноду, где local cache готов и модель помещается. Ноды без
подходящего диска не надо помечать `ai.deckhouse.io/model-cache=true`.
PVC/materialize fallback в workload namespaces больше не является supported
path: если workload попадёт на неподготовленную ноду, CSI mount будет
fail/wait через kubelet events.
